...
# Training settings
parser = argparse.ArgumentParser(description='Saajnet v2 (Pytorch)')
...
save_path='results/saajnetv2-w{}a{}.pt'.format(args.wb, args.ab)
prev_acc = 0
...
class inception_module(nn.Module):
	def __init__(self, ifm_ch, ofm_ch):
		super(inception_module, self).__init__()
		self.conv1x1 = BinarizeConv2d_block(args.wb, ifm_ch, ofm_ch, kernel_size=1, stride=1, padding=0, bias=True)
		self.conv3x3 = BinarizeConv2d_block(args.wb, ifm_ch, ofm_ch, kernel_size=3, stride=1, padding=1, bias=True)
		self.conv5x5 = BinarizeConv2d_block(args.wb, ifm_ch, ofm_ch, kernel_size=5, stride=1, padding=2, bias=True)
        self.conv7x7 = BinarizeConv2d_block(args.wb, ifm_ch, ofm_ch, kernel_size=7, stride=1, padding=3, bias=True)
	...
	def forward(self, x):
		...
		x = torch.cat((x_1x1, x_3x3, x_5x5, x_7x7), 1)
		return x

class SaajNet(nn.Module):
    def __init__(self):
        super(SaajNet, self).__init__()
        self.features = nn.Sequential(
            BinarizeConv2d(args.wb, 1, 8, kernel_size=3, stride=1, padding=1, bias=True),
            nn.BatchNorm2d(8),
            nn.Hardtanh(inplace=True),
            Quantizer(args.ab),
            nn.MaxPool2d(kernel_size=2, stride=2),

            inception_module(8,16),
        	nn.MaxPool2d(kernel_size=2, stride=2),

        	inception_module(64,32),
        	nn.MaxPool2d(kernel_size=2, stride=2),

        	inception_module(128,64),
        	nn.MaxPool2d(kernel_size=2, stride=2),
        	)

        self.classifier = nn.Sequential(
            BinarizeLinear(args.wb, 1*1*4*64, 1024, bias=True),
            nn.BatchNorm1d(1024),
            nn.Hardtanh(inplace=True),
            Quantizer(args.ab),
    ...
    def export(self):
        ...
        for k in range(len(self.features)):
            ...
            if hasattr(self.features[k], 'conv7x7'):
                for j in range(len(self.features[k].conv7x7.features)):
                    if hasattr(self.features[k].conv7x7.features[j], 'weight') and not hasattr(self.features[k].conv7x7.features[j], 'running_mean'):
                        dic['arr_'+str(i)] = self.features[k].conv7x7.features[j].weight.detach().numpy()
                        i = i + 1
                        dic['arr_'+str(i)] = self.features[k].conv7x7.features[j].bias.detach().numpy()
                        i = i + 1
                    elif hasattr(self.features[k].conv7x7.features[j], 'running_mean'):
                        dic['arr_'+str(i)] = self.features[k].conv7x7.features[j].bias.detach().numpy()
                        i = i + 1
                        dic['arr_'+str(i)] = self.features[k].conv7x7.features[j].weight.detach().numpy()
                        i = i + 1
                        dic['arr_'+str(i)] = self.features[k].conv7x7.features[j].running_mean.detach().numpy()
                        i = i + 1
                        dic['arr_'+str(i)] = 1./np.sqrt(self.features[k].conv7x7.features[j].running_var.detach().numpy())
                        i = i + 1
        ...
        save_file = 'results/saajnetv2-w{}a{}.npz'.format(args.wb, args.ab)
