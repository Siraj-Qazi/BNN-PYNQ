...
# Training settings
parser = argparse.ArgumentParser(description='Saajnet Lite (Pytorch)')
...
save_path='results/saajnet-lite-w{}a{}.pt'.format(args.wb, args.ab)
prev_acc = 0
...
class SaajNet(nn.Module):
    def __init__(self):
        ...
        self.features = nn.Sequential(
        	...
        	inception_module(48,32),
        	nn.MaxPool2d(kernel_size=2, stride=2),

        	inception_module(96,64),
        	nn.MaxPool2d(kernel_size=2, stride=2),
        	)

        self.classifier = nn.Sequential(
            BinarizeLinear(args.wb, 1*1*3*64, 1024, bias=True),
            nn.BatchNorm1d(1024),
            nn.Hardtanh(inplace=True),
            Quantizer(args.ab),
    ...
    def export(self):
        ...
        save_file = 'results/saajnet-lite--w{}a{}.npz'.format(args.wb, args.ab)
