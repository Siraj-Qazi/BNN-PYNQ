...
# Training settings
parser = argparse.ArgumentParser(description='PyTorch Quantized Resnet XT (MNIST)')
...
save_path='results/resnetXT-w{}a{}.pt'.format(args.wb, args.ab)
prev_acc = 0
...
class resnet_module(nn.Module):
    def __init__(self):
        ...
        self.features = nn.Sequential(
            BinarizeConv2d(args.wb, 64, 64, kernel_size=3, stride=1, padding=1, bias=True),
            nn.BatchNorm2d(64),
    ...
    def export(self):
        ...
        for k in range(len(self.features)):
            if hasattr(self.features[k], 'weight') and not hasattr(self.features[k], 'running_mean'):
                ...
                i = i + 1
            # process resnet block
            elif hasattr(self.features[k], 'features'):
                for j in range(len(self.features[k].features)):
                    if hasattr(self.features[k].features[j], 'weight') and not hasattr(self.features[k].features[j], 'running_mean'):
                        dic['arr_'+str(i)] = self.features[k].features[j].weight.detach().numpy()
                        i = i + 1
                        dic['arr_'+str(i)] = self.features[k].features[j].bias.detach().numpy()
                        i = i + 1
                    elif hasattr(self.features[k].features[j], 'running_mean'):
                        dic['arr_'+str(i)] = self.features[k].features[j].bias.detach().numpy()
                        i = i + 1
                        dic['arr_'+str(i)] = self.features[k].features[j].weight.detach().numpy()
                        i = i + 1
                        dic['arr_'+str(i)] = self.features[k].features[j].running_mean.detach().numpy()
                        i = i + 1
                        dic['arr_'+str(i)] = 1./np.sqrt(self.features[k].features[j].running_var.detach().numpy())
                        i = i + 1
        ...
        save_file = 'results/resnetXT-w{}a{}.npz'.format(args.wb, args.ab)
